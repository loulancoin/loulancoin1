...
const string strMessageMagic = "loulan Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    ...
    if (pindexBest->nTime >= 1467008492) // Thu, 01 Sep 2016 00:00:00 UTC
    {
        // Year 2. 5% interest and MAX_STAKE_BLOCK_REWARD = 2.5
        nSubsidy = nCoinAge * 4 * CENT * 33 / (365 * 33 + 8);
        nSubsidyLimit = 100 * COIN;
    }
    if (pindexBest->nTime >= 1498544492) // Sun, 01 Sep 2017 00:00:00 UTC
    {
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound: invalid block=%s  height=%d  trust=%s  blocktrust=%d  date=%s\n",
      pindexNew->GetBlockHash().ToString().substr(0,20).c_str(), pindexNew->nHeight,
      CBigNum(pindexNew->nChainTrust).ToString().c_str(), nBestInvalidBlockTrust.Get64(),
      DateTimeStrFormat("%x %H:%M:%S", pindexNew->GetBlockTime()).c_str());
    printf("InvalidChainFound:  current best=%s  height=%d  trust=%s  blocktrust=%d  date=%s\n",
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    printf("REORGANIZE: Disconnect %u blocks; %s..%s\n", vDisconnect.size(), pfork->GetBlockHash().ToString().substr(0,20).c_str(), pindexBest->GetBlockHash().ToString().substr(0,20).c_str());
    printf("REORGANIZE: Connect %u blocks; %s..%s\n", vConnect.size(), pfork->GetBlockHash().ToString().substr(0,20).c_str(), pindexNew->GetBlockHash().ToString().substr(0,20).c_str());
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    else
    {
        ...
        if (!vpindexSecondary.empty())
            printf("Postponing %u reconnects\n", vpindexSecondary.size());
    ...
    printf("SetBestChain: new best=%s  height=%d  trust=%s  blocktrust=%d  date=%s\n",
      hashBestChain.ToString().substr(0,20).c_str(), nBestHeight,
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (fDebug && GetBoolArg("-printcoinage"))
            printf("coin age nValueIn=%d nTimeDiff=%d bnCentSecond=%s\n", nValueIn, nTime - txPrev.nTime, bnCentSecond.ToString().c_str());
    }
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        ...
        {
            if (txCoinStake.nTime >= pindexBest->GetBlockTime()+1)
            {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "loulan", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        const char* pszTimestamp = "LouLanCoin is me!";
        CTransaction txNew;
        txNew.nTime = 1482030599;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe63f56f415ed73f7ea7e94018fb5ecad1cc0347bcf48868c2a749699e8d63392"));
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  mint %7s  tx %u",
            pindex->nHeight,
            pindex->nFile,
            pindex->nBlockPos,
            block.GetHash().ToString().c_str(),
...
unsigned char pchMessageStart[4] = { 0xf3, 0xf9, 0xf4, 0x3c };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "getdata")
    {
        ...
        if (fDebugNet || (vInv.size() != 1))
            printf("received getdata (%u invsz)\n", vInv.size());
    ...
    else if (strCommand == "pong")
    {
        ...
        if (!(sProblem.empty())) {
            printf("pong %s %s: %s, %x expected, %x received, %zu bytes\n"
                , pfrom->addr.ToString().c_str()
